...
@sensors_router.post("/preadd/{device_id}", summary="Sent data a the database ✔", response_model=Sensors)
async def sent_sensor_data(device_id:UUID,sensors: SensorSent, current_user:Persona = Depends(get_current_user)):
    ...
    #     raise HTTPException(
    #         status_code=500,
    #         detail=f"Error al cargar el sensor: {str(e)}",
    #     )

@sensors_router.get("/getitems/", summary="Get data sensor", response_model=List[SensorsDetail])
...
@sensors_router.get("/history/", summary="Get history Sensors 2", response_model=List[Historydata])
...
@sensors_router.put("/{id_sensor}", summary="update sensor", response_model=SensorsDetail)
async def update(id_sensor:UUID, data:SensorUpdate, current_user:Persona =Depends(get_current_user)):
    await check_admin_role(current_user)
    try:
