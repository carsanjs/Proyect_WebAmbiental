...
const ChartTwo: React.FC = () => {
  ...
  const prepareChartData = () => {
    const groupedData: { [key: string]: SensorData[] } = data.reduce((acc, current) => {
        if (!acc[current.sensor_id]) {
          acc[current.sensor_id] = [];
        }
        acc[current.sensor_id].push(current.data);
        return acc;
      },
      {}
    );

    const series = Object.keys(groupedData).map((sensor_id) => ({
      name: sensor_id,
      data: groupedData[sensor_id].map((d) => [
        d["temperatura (C):"],
        d["temperatura (F):"],
        d["humedad"],
      ]),
