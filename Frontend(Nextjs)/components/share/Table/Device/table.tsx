...
import { fetchDevices } from "@/services/axios";
...
export default async function InvoicesTable() {
  ...
  const fectInvoices = async () => {
    let response = await fetchDevices();
              ...
              <div className="rounded-lg bg-gray-50 p-2 md:pt-0">
                <div className="md:hidden">
                  {invoices?.map((invoice) => (
                    <div
                      key={invoice.id_device}
                      className="mb-2 w-full rounded-md bg-white p-4"
                    >
                      <div className="flex items-center justify-between border-b pb-4">
                        <div>
                          <div className="mb-2 flex items-center">
                            <p>{invoice.name_device}</p>
                          </div>
                      ...
                      <div className="flex w-full items-center justify-between pt-4">
                        ...
                        <div className="flex justify-end gap-2">
                          <UpdateInvoice id={invoice.id_device} />
                          <DeleteInvoice id={invoice.id_device} onDeleteSuccess={handleDeleteSuccess}/>
                        </div>
                      </div>
                    </div>
                  ))}
