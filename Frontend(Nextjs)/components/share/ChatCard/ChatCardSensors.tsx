    ...
    const fetchDeviceName = async () => {
      try {
        for (const sensor of sensors) {
          const response = await axiosInstance.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/device/detail/${sensor.device_id}`
          );
          const deviceName = response.data.name_device;
          console.log(deviceName);
          console.log(response.data);
          setDeviceNameMap((prevMap) => ({...prevMap,[sensor.device_id]: deviceName}));
